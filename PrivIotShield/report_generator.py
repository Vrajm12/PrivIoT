...
def generate_report(scan, report_type):
    ...
    try:
        ...
        # Try to use the enhanced AI report generation with fallback
        try:
            # Use the enhanced OpenAI integration
            logger.info(f"Generating {report_type} report using enhanced AI")
            html_content = generate_report_with_ai(report_data, report_type)
            return Markup(html_content)
        except Exception as e:
            logger.warning(f"Enhanced AI report generation failed: {e}. Falling back to standard generation.")
            
            # Generate appropriate report based on type with standard methods
